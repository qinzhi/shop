...
class CommonDao4sSelf {
	...
	public function __construct($ip,$dbname,$user,$passwd) {
			   ...
			   if(isset( $mydbpools[$dbkey] )){
			        ...
			        $this->MyDB  =	&ADONewConnection('mysql', 'pear');
				   	$this->MyDB->NConnect($ip, $user,$passwd, $dbname);
				   	$this->MyDB->debug_console = true;
				   	$this->MyDB->Query("SET NAMES {$_CFG['DBCharset']}");
