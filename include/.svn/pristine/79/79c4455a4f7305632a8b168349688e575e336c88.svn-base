...
class CommonDao4Mysqli {
	...
	function __construct() {
		 ...
		 $mysqldbi = $_REQUEST['mysqlidbconnectsign'];
		 ...
		 if( empty($mysqldbi)  ){
				...
				$host = $_CFG['DB']['Host'];
				$user = $_CFG['DB']['User'];
				$pwd = $_CFG['DB']['Password'];
				$dbname = $_CFG['DB']['Database'];
				$charset = $_CFG['DBCharset'];
			   
			   self::$db = new mysqli( $host , $user ,$pwd , $dbname);
			   ...
			   $_REQUEST['mysqlidbconnectsign'] =   self::$db;
