...
class CommonDao{
    ...
    public function __construct() {
        ...
        if(isset( $_REQUEST['dbconnectsign'] )){
            $MyDB =  $_REQUEST['dbconnectsign'];
        }else{

            $MyDB->NConnect($_CFG['DB']['Host'], $_CFG['DB']['User'], $_CFG['DB']['Password'], $_CFG['DB']['Database']);
            $MyDB->debug_console = true;
            $MyDB->Query("SET NAMES {$_CFG['DBCharset']}");
            $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
            $_REQUEST['dbconnectsign'] = $MyDB;
